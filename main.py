...
if "✅" in item: st.success(item)
    ...
    # =====================================================
# ADD-ON ONLY (NO CHANGE TO ABOVE CODE)
# =====================================================

st.write("---")
st.subheader("🧪 Demo / Auto Input")

colA, colB = st.columns(2)

if colA.button("⚡ AUTO RANDOM 10"):
    st.session_state.temp_input = [random.choice(["B", "S"]) for _ in range(10)]
    st.session_state.show_res = False
    st.success("✅ Auto Random 10 Loaded")

if colB.button("📌 LOAD DEMO PATTERN"):
    demo_patterns = [
        ["B","B","S","B","S","B","S","B","B","S"],
        ["S","S","B","S","B","S","B","S","S","B"],
        ["B","S","B","S","B","S","B","S","B","S"]
    ]
    st.session_state.temp_input = random.choice(demo_patterns)
    st.session_state.show_res = False
...
try:
    df_hist = pd.read_sql_query(
        "SELECT result, COUNT(*) as total FROM history GROUP BY result",
        conn
    )
    st.dataframe(df_hist, use_container_width=True)
except Exception as e:
    st.info("ডাটা আসলে এখানে দেখাবে")

st.write("---")
st.caption(
    "⚠️ Educational & demonstration purpose only. "
    "No real game/server is accessed. No guarantee of profit."
)
